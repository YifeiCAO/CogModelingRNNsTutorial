...
class BiRNN(hk.RNNCore):
  ...
  def __init__(self, rl_params, network_params, init_value=0.5):
    ...
    # Initialize LSTM cells for value and habit networks
  ...
  def _value_rnn(self, state, value, action, reward):
    ...
    update = hk.Linear(1)(next_state.hidden)
  ...
  def _habit_rnn(self, state, habit, action):
    ...
    next_habit = hk.Linear(self._n_actions)(next_state.hidden)
  ...
  def initial_state(self, batch_size: Optional[int]):
    return (
        ...
        self.value_lstm.initial_state(batch_size),  # v_state
