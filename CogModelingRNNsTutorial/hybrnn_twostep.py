...
def _parse_twostep_inputs(inputs: jnp.ndarray, n_actions: int):
  """Parse two-step inputs as [action, reward, transition]."""
  action = inputs[:, 0].astype(jnp.int32)
  reward = inputs[:, 1]
  transition = inputs[:, 2]
  action_onehot = jax.nn.one_hot(action, n_actions)
  return action, action_onehot, reward, transition
...
class BiRNN(hk.RNNCore):
  ...
  def _value_rnn(self, state, value, action, reward, transition):

    pre_act_val = jnp.sum(value * action, axis=1)  # (batch_s, 1)

    inputs = jnp.concatenate(
        [
            pre_act_val[:, jnp.newaxis],
            reward[:, jnp.newaxis],
            transition[:, jnp.newaxis],
        ],
        axis=-1,
    )
  ...
  def _habit_rnn(self, state, habit, action, transition):

    inputs = jnp.concatenate([action, transition[:, jnp.newaxis]], axis=-1)
  ...
  def __call__(self, inputs: jnp.ndarray, prev_state: jnp.ndarray):
    ...
    _, action_onehot, reward, transition = _parse_twostep_inputs(
        inputs, self._n_actions
    )
    
    # Value module: update/create new values
    next_value, next_v_state = self._value_rnn(
        v_state, value, action_onehot, reward, transition
    )

    # Habit module: update/create new habit
    next_habit, next_h_state = self._habit_rnn(
        h_state, habit, action_onehot, transition
    )
...
class BiControlRNN(hk.RNNCore):
    ...
    def _learnable_gate_mech(
        self, state, value, action_onehot, reward, transition
    ):
        """
        ...
        transition_expanded = transition[:, None]
    ...
    def _value_rnn(self, state, value, action_onehot, reward, transition):
        with hk.name_scope('value_module'):
            # 1. 获取两路隐状态和门控信号
            h_ctx, h_mem, gate_val = self._learnable_gate_mech(
                state, value, action_onehot, reward, transition
            )
    ...
    def _habit_rnn(
        self, state, habit, action_onehot, value, reward, transition
    ):
        """
    ...
    def __call__(self, inputs: jnp.ndarray, prev_state: Tuple):
        ...
        # Value Module
        next_value, next_v_state, v_gate = self._value_rnn(
            v_state, value, action_onehot, reward, transition
        )

        # Habit Module
        next_habit, next_h_state, h_gate = self._habit_rnn(
            h_state, habit, action_onehot, value, reward, transition
        )
...
class BiAdditiveRNN(hk.RNNCore):
    """
    A Learnable Gated Hybrid RNN (Additive Version).
    Context and Memory channels are added directly instead of being weighted.
    """
    ...
    def _learnable_gate_mech(
        ...
        Separated Stream Processing:
        Returns (h_context, h_memory, gate_signal) so we can compute distinct updates.
        Note: gate_signal is computed but not used for weighting in this version.
        """
        # 1. Input Preparation
        reward_expanded = reward[:, None]
        transition_expanded = transition[:, None]
    ...
    def _value_rnn(self, state, value, action_onehot, reward, transition):
        with hk.name_scope('value_module'):
            ...
            # =========================================================
            # 核心修改：Additive Combination (直接相加)
            # =========================================================
    ...
    def _habit_rnn(
        ...
        with hk.name_scope('habit_module'):
            ...
            # 融合 Habit：直接相加
            next_habit = update_ctx + update_mem
            
            # 更新 hidden state：直接相加
            mixed_hidden = h_ctx + h_mem
            next_state = jax.nn.tanh(mixed_hidden)
...
class BiRNN_oneDim(hk.RNNCore):
  ...
  def _habit_rnn(self, state, habit, action, transition):
    ...
    inputs = jnp.concatenate(
        [habit[:, jnp.newaxis], action[:, jnp.newaxis], transition[:, jnp.newaxis]], axis=-1)
    if self._ho:  # "o" = output -> feed previous output back in
      inputs = jnp.concatenate([inputs, habit], axis=-1)
    if self._hs:  # "s" = state -> feed previous hidden state back in
      inputs = jnp.concatenate([inputs, state], axis=-1)
  ...
  def __call__(self, inputs: jnp.ndarray, prev_state: jnp.ndarray):
    h_state, v_state, habit, value = prev_state
    action, _, reward, transition = _parse_twostep_inputs(
        inputs, self._n_actions
    )
    
    # Value module: update/create new values
    next_value, next_v_state = self._value_rnn(
        v_state, value, action, reward, transition
    )

    # Habit module: update/create new habit
    next_habit, next_h_state = self._habit_rnn(
        h_state, habit, action, transition
    )
...
class BiConRNN(hk.RNNCore):
  ...
  def _value_con_rnn(self, state, value, action, reward, transition):

    counter_action = 1.0 - action
    pre_act_val = jnp.sum(value * counter_action, axis=1)  # (batch_s, 1)
    ...
    update = hk.Linear(1)(next_state)
    value = (1 - self.forget) * value + self.forget * self.init_value
    next_con_value = value + counter_action * update
  ...
  def __call__(self, inputs: jnp.ndarray, prev_state: jnp.ndarray):
    ...
    next_c_value, next_c_state = self._value_con_rnn(
        c_state, c_value, action_onehot, reward, transition
    )
...
class BiChannelRNN(hk.RNNCore):
  """Two-channel hybrid RNN for the TWO-STEP task (configurable gate).

  Two-step counterpart of hybrnn.BiChannelRNN: inputs are parsed as
  [action, reward, transition] and `transition` is fed to both channels and to
  the gate alongside reward / action / value.
  ...
  Combination modes:
    ...
    gate_mode='learnable' : gate = sigmoid(MLP([reward, transition, action, value, memory_state]))
    gate_mode='surprise'  : gate = sigmoid(MLP([reward, transition, pre_act_val, RPE]))
    gate_mode='hard'      : like 'learnable' but the gate is binarised to {0,1} (straight-through)
  For 'learnable'/'surprise'/'hard': final_update = gate*update_ctx + (1-gate)*update_mem.

  HABIT module -- a single recurrent (memory) channel over [action, transition]
  ...
  def _combine_gate(self, memory_state, value, action_onehot, reward, transition):
    ...
    with hk.name_scope('gate'):
      if self._gate_mode in ('learnable', 'hard'):
        ...
        gate_inputs = jnp.concatenate(
            [reward[:, None], transition[:, None], action_onehot, value,
             memory_state], axis=-1)
        ...
        gate_inputs = jnp.concatenate(
            [reward[:, None], transition[:, None], pre_act_val, rpe], axis=-1)
  ...
  def _value_rnn(self, memory_state, value, action_onehot, reward, transition):
    with hk.name_scope('value_module'):
      evidence = jnp.concatenate(
          [reward[:, None], transition[:, None], action_onehot, value], axis=-1)
      ...
      gate = self._combine_gate(memory_state, value, action_onehot, reward, transition)
  ...
  def _habit_rnn(self, h_state, action_onehot, transition):
    """Habit = one recurrent memory channel over [action, transition] only."""
    with hk.name_scope('habit_module'):
      mem_in = jnp.concatenate(
          [action_onehot, transition[:, None], h_state], axis=-1)
  ...
  def __call__(self, inputs: jnp.ndarray, prev_state: Tuple):
    ...
    next_value, next_memory_state, _gate = self._value_rnn(
        memory_state, value, action_onehot, reward, transition)
    next_habit, next_h_state = self._habit_rnn(h_state, action_onehot, transition)
